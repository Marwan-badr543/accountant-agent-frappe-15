...
from razyyn.agent_api.db.agent_api_repository import (
	doctype_exists,
...
def _max_result_rows() -> int:
	return int(frappe.conf.get("razyyn_max_query_rows") or DEFAULT_MAX_RESULT_ROWS)


def _query_timeout_seconds() -> int:
	return int(frappe.conf.get("razyyn_query_timeout_seconds") or DEFAULT_QUERY_TIMEOUT_SECONDS)
...
def validate_and_execute_query(sql_query: str, settings_user: str) -> dict:
	...
	return {
		...
		# do not exist. The site owns this number — `razyyn_max_query_rows`
