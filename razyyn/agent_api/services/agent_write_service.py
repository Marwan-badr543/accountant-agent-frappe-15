...
from razyyn.agent_api.db.agent_write_repository import (
	amend_document,
	cancel_document,
	commit_write_log,
...
AGENT_USER: str = "razyyn@agent.local"
...
def assert_session_is_agent_user() -> str:
	...
	if session_user != AGENT_USER:
		raise NotAgentSessionError(_("This endpoint may only be used by the Razyyn account."))
	return session_user
...
def load_write_policy() -> WritePolicy:
	...
	return WritePolicy(
		...
		allowed_companies=tuple(row.company for row in (doc.allowed_companies or []) if row.company),
		blocked_accounts=tuple(row.account for row in (doc.blocked_accounts or []) if row.account),
	)
	...
	if delta > policy.posting_date_max_days_forward:
		raise PolicyCapExceededError(
			...
			if not policy.posting_date_max_days_forward
				...
				"The date {0} is more than {1} days ahead, which is outside the " "window this system allows."
...
def _assert_company_allowed(policy: WritePolicy, payload: dict) -> None:
	...
	if company and company not in policy.allowed_companies:
		raise PolicyCapExceededError(_("The agent is not permitted to post in {0}.").format(company))
...
def _assert_no_blocked_accounts(policy: WritePolicy, payload: dict, doctype: str) -> None:
	...
	if offending:
		raise PolicyCapExceededError(
			_("The agent is not permitted to post to {0} in this system.").format(", ".join(offending))
		)
...
def assert_run_caps(policy: WritePolicy, document_count: int, total_amount: float) -> None:
	...
	if policy.max_documents_per_run and document_count > policy.max_documents_per_run:
		raise PolicyCapExceededError(
			_("This run would create {0} documents, above the limit of {1} set " "in this system.").format(
				document_count, policy.max_documents_per_run
			)
		)
	if policy.max_total_amount_per_run and abs(total_amount) > policy.max_total_amount_per_run:
		raise PolicyCapExceededError(
			_("This run totals {0}, above the per-run limit of {1} set in this " "system.").format(
				total_amount, policy.max_total_amount_per_run
			)
		)
...
def _search_one_reference(ref: dict, limit: int) -> dict:
	...
	if not target_doctype or not doctype_exists(target_doctype):
		return {
			...
			"total_matched": 0,
			"truncated": False,
			"error_code": "UNKNOWN_REFERENCE_KIND",
		}

	meta = get_doctype_meta(target_doctype)
	available = {df.fieldname for df in meta.fields} | {"name"}

	search_fields = [f for f in ((spec or {}).get("search_fields") or ("name",)) if f in available] or [
		"name"
	]
	display_fields = [f for f in ((spec or {}).get("display_fields") or ("name",)) if f in available] or [
		"name"
	]

	filters = {
		key: value for key, value in ((spec or {}).get("base_filters") or {}).items() if key in available
	}
	filters.update(_context_filters(context, available))

	or_filters = [[sf, "like", f"%{raw_value}%"] for sf in search_fields] if raw_value else None
...
def _preflight_links(doc: Any) -> list[PreflightFinding]:
	...
	for child in doc.get_all_children():
		try:
			child_invalid, child_cancelled = child.get_invalid_links(is_submittable=doc.meta.is_submittable)
		except Exception:
			continue
...
def _preflight_frozen_period(payload: dict) -> list[PreflightFinding]:
	...
	return [
		PreflightFinding(
			...
			human_message=_(
				"The books in this system are closed up to {0}, so an entry " "dated {1} cannot be recorded."
...
def _preflight_transactional(doc: Any) -> tuple[list[PreflightFinding], bool]:
	...
	try:
		...
		findings.append(
			PreflightFinding(
				"BLOCKING", "PERMISSION_DENIED", "", None, _user_message("PERMISSION_DENIED", str(exc))
			)
		)
...
_FROZEN_PERIOD_PATTERN = re.compile(r"not authorized to add or update entries before", re.IGNORECASE)
...
_DEFAULT_MESSAGE_BY_CODE: dict[str, str] = {
	...
	"A System Manager can grant it on the Razyyn user.",
...
def list_agent_documents(limit: int = 20, target_doctype: str | None = None) -> dict:
	...
	for row in rows:
		...
		if state is None:
			...
			continue
		documents.append(
			{**state, "written_at": str(row.get("creation") or ""), "approved_by": row.get("approved_by")}
		)
...
def alert_on_stranded_in_flight() -> None:
	...
	"""
	from razyyn.agent_api.db.agent_write_repository import find_stranded_in_flight
