...
def get_agent_server_url() -> str:
	...
	return (frappe.conf.get("razyyn_server_url") or _DEFAULT_AGENT_SERVER_URL).rstrip("/")
...
class AgentSettings(Document):
	...
	def _assert_email_not_already_connected(self) -> None:
		...
		if duplicate:
			frappe.throw(
				_("An Razyyn connection already exists for {0}.").format(self.email),
