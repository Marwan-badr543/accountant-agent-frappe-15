...
class ChatSessionManager {
	...
	async load_chats(reload_active = true) {
		try {
			this.chats = await frappe.xcall("razyyn.razyyn.page.agent_chat.agent_chat.get_chats");
	...
	rename_chat(session_id, current_title) {
		frappe.prompt(
			...
			async (values) => {
				...
				try {
					await frappe.xcall(
						"razyyn.razyyn.page.agent_chat.agent_chat.update_chat_title",
	...
	delete_chat_session(session_id) {
		frappe.confirm(
			...
			async () => {
				...
				try {
					await frappe.xcall("razyyn.razyyn.page.agent_chat.agent_chat.delete_chat", {
	...
	async load_chat_history() {
		...
		try {
			let messages = await frappe.xcall(
				"razyyn.razyyn.page.agent_chat.agent_chat.get_chat_history",
