frappe.pages["agent-chat"].on_page_load = function (wrapper) {
			...
			if (window.mermaid) {
				mermaid.initialize({
					...
					themeVariables: {
						...
						xyChart: {
							plotColorPalette:
								"#10a37f, #3b82f6, #f59e0b, #8b5cf6, #ec4899, #ef4444, #06b6d4, #14b8a6",
							titleColor: "#111827",
	...
	frappe.require(
		[
			"assets/razyyn/js/agent_selector.js",
			"assets/razyyn/js/file_upload_handler.js",
			"assets/razyyn/js/chat_attachments_renderer.js",
			"assets/razyyn/js/chat_ui_manager.js",
			"assets/razyyn/js/chat_session_manager.js",
			"assets/razyyn/js/chat_message_handler.js",
		],
		() => {
			new RazyynChat(wrapper, page);
		}
	);
};
...
class RazyynChat {
	constructor(wrapper, page) {
		...
		frappe.realtime.on("agent_message_chunk", (data) => {
			if (data && data.session_id) {
				...
				if (data.session_id === this.session_manager.session_id) {
					this.ui_manager.update_stream_bubble(
						this.msg_box,
						stream.bubble_id,
						stream.accumulated
					);
		...
		frappe.realtime.on("agent_message_reasoning", (data) => {
			if (data && data.session_id) {
				...
				if (data.session_id === this.session_manager.session_id) {
					this.ui_manager.update_stream_reasoning(
						this.msg_box,
						stream.bubble_id,
						stream.reasoning
					);
		...
		frappe.realtime.on("agent_node_start", (data) => {
			if (data && data.session_id) {
				...
				if (data.session_id === this.session_manager.session_id) {
					this.ui_manager.update_stream_status(
						this.msg_box,
						stream.bubble_id,
						display,
						stream.steps
		...
		frappe.realtime.on("agent_tool_start", (data) => {
			if (data && data.session_id) {
				...
				let display =
					data.label || tool_display_names[data.tool] || __("Working on it...");
				stream.status = display;
				...
				if (this.active_streams && this.active_streams[data.session_id]) {
					...
					if (data.session_id === active_session_id) {
						this.ui_manager.finalize_stream_bubble(
							...
							stream.bubble_id,
							`⚠️ **Error:** ${
								data.error || __("An error occurred during execution.")
							}`,
							new Date().toISOString(),
							__("Failed")
						);
	...
	async init() {
		...
		this.container = $('<div class="agent-chat-container"></div>').appendTo(
	...
	async check_connection() {
		...
		try {
			...
			let res = await frappe.xcall(
				"razyyn.razyyn.page.agent_chat.agent_chat.get_connection_status",
		...
		let card_html = `
			<div class="agent-auth-card">
				...
				<ul class="nav nav-tabs d-flex justify-content-center">
					<li class="nav-item">
						<a class="nav-link ${this.active_tab === "login" ? "active" : ""}" data-tab="login">${__(
			"Login"
		)}</a>
					</li>
					<li class="nav-item">
						<a class="nav-link ${this.active_tab === "signup" ? "active" : ""}" data-tab="signup">${__(
			"Sign Up"
		)}</a>
					</li>
				</ul>
				...
				<div class="auth-form-container">
					<form id="agent-auth-form">
						<div class="form-group signup-field" style="display: ${
							this.active_tab === "signup" ? "block" : "none"
						};">
	...
	setup_auth_events($card) {
		...
		$card.find("#agent-auth-form").on("submit", async (e) => {
			...
			frappe.dom.freeze(
				this.active_tab === "login" ? __("Connecting...") : __("Creating Account...")
			);

			try {
				let res = await frappe.xcall(
					"razyyn.razyyn.page.agent_chat.agent_chat.authenticate_agent",
					{
	...
	async render_chat_view() {
					...
					<div class="agent-chat-header">
						<div class="agent-status-container">
							<div class="agent-status-badge">
								...
								${__("Connected:")} <a href="javascript:void(0)" class="agent-email-link" title="${__(
			"Click to view Agent Settings"
		)}"><strong>${
			this.connected_email
		}</strong> <i class="fa fa-cog" style="font-size: 11px; margin-left: 3px;"></i></a>
							</div>
						</div>
						<div class="agent-header-actions" style="display: flex; align-items: center; gap: 12px;">
							...
							<button class="agent-theme-toggle-btn btn btn-xs btn-default" style="display: flex; align-items: center; justify-content: center; width: 28px; height: 28px; padding: 0; border-radius: 6px;" title="${__(
								"Toggle Theme"
							)}">
					...
					<div class="agent-input-container">
						...
						<div class="agent-input-card">
							<textarea class="agent-textarea" placeholder="${__(
								"Type your financial question or query here..."
							)}" id="agent-input-msg" maxlength="10000"></textarea>
		...
		this.file_upload_handler.init(this.layout.find(".agent-input-container"), this.textarea);
		...
		this.layout.find(".agent-email-link").on("click", async (e) => {
			...
			try {
				let doc_name = await frappe.xcall(
					"razyyn.razyyn.doctype.agent_settings.agent_settings.get_agent_settings_name",
				...
				if (doc_name) {
					...
					frappe.show_alert({
						message: __("Agent Settings record not found."),
						indicator: "orange",
					});
				}
			} catch (err) {
		...
		this.layout.find(".agent-settings-btn").on("click", () => {
							...
							<div style="display: flex; flex-direction: column; gap: 10px;">
								<button class="btn btn-default btn-block logout-action-btn" style="text-align: left; display: flex; align-items: center; gap: 8px; padding: 10px 15px; margin: 0;">
									...
									<div>
										...
										<span style="font-size: 11px; color: var(--text-muted); font-weight: normal;">${__(
											"Disconnect the agent temporarily."
										)}</span>
									</div>
								</button>
								<button class="btn btn-danger btn-block delete-action-btn" style="text-align: left; display: flex; align-items: center; gap: 8px; padding: 10px 15px; background-color: var(--bg-red-light, #fff5f5); border-color: var(--border-red, #ffcccc); color: var(--text-red, #c53030); margin: 0;">
									...
									<div>
										...
										<span style="font-size: 11px; color: var(--text-red, #c53030); opacity: 0.8; font-weight: normal;">${__(
			...
			d.$wrapper.find(".logout-action-btn").on("click", async () => {
				...
				try {
					let agent_email = localStorage.getItem("connected_agent_email");
					await frappe.xcall(
						"razyyn.razyyn.page.agent_chat.agent_chat.disconnect_agent",
						{ agent_email: agent_email }
					);
				...
				frappe.confirm(
					...
					async () => {
						...
						try {
							let agent_email = localStorage.getItem("connected_agent_email");
							await frappe.xcall(
								"razyyn.razyyn.page.agent_chat.agent_chat.delete_agent_account",
								{ agent_email: agent_email }
							);
							self.connected = false;
							self.connected_email = null;
