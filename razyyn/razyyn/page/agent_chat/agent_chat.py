...
def register_agent_on_server(email: str, password: str, company_name: str, api_key: str) -> None:
	...
	try:
		...
		frappe.log_error(f"Agent registration request error: {e!s}", "Razyyn Auth")
...
def login_agent_on_server(email: str, password: str) -> str:
	...
	try:
		...
		frappe.log_error(f"Agent login request error: {e!s}", "Razyyn Auth")
		frappe.throw(_("Could not connect to Agent Server. Please make sure the server is running."))
...
def refresh_agent_token_on_server(access_token: str) -> str:
	...
	try:
		...
		frappe.log_error(f"Agent token refresh request error: {e!s}", "Razyyn Refresh")
	return None
		...
		for msg in plans:
			if msg.content and msg.content.startswith('{"type": "plan"'):
				try:
					...
					frappe.log_error(
						f"Error deprecating plan message {msg.name}: {json_err!s}", "Razyyn Deprecate Plan"
					)
		frappe.db.commit()
	except Exception as e:
		frappe.log_error(f"Error in deprecate_previous_plans: {e!s}", "Razyyn Deprecate Plan")
...
def save_chat_history(session_id: str, sender: str, content: str) -> None:
	...
	try:
		...
		frappe.log_error(f"Error saving user message to history: {e!s}", "Razyyn Chat")
...
def save_chat_event_if_not_duplicate(session_id: str, sender: str, content: str) -> None:
	...
	try:
		...
		frappe.log_error(f"Error checking/saving chat event: {e!s}", "Razyyn Chat Event")
	...
	try:
		if file_urls:
			for url in file_urls:
				...
				files_list.append(("files", (_original_filename(os.path.basename(file_path)), handle)))
...
def get_connection_status(agent_email=None):
	...
	try:
		...
		frappe.log_error(f"Error checking connection status: {e!s}", "Razyyn Connect")
		...
		if msg.content and msg.content.startswith('{"type": "plan"'):
			if lock:
				try:
					...
					frappe.log_error(f"Database lock timeout or error: {e!s}", "Razyyn Plan Lock")
...
def send_message(message, session_id, agent_email, agent_type="auto", file_urls=None):
	...
	if latest_plan:
		try:
			...
			frappe.log_error(f"Error updating plan status JSON: {e!s}", "Razyyn Plan Status Update")
	...
	frappe.enqueue(
		"razyyn.razyyn.page.agent_chat.agent_chat.process_agent_message_background",
		queue="long",
...
def process_agent_message_background(
	...
	if not doc:
		...
		frappe.log_error(error_msg, "Razyyn Stream")
	...
	try:
		...
		update_chat_timestamp(session_id)
		frappe.log_error(
			f"Error processing agent message in background: {error_msg}", "Razyyn Chat Background"
		)
		frappe.publish_realtime(
			event="agent_message_error",
			message={"session_id": session_id, "error": error_msg},
...
def cancel_agent(session_id, agent_email):
	...
	try:
		...
		frappe.log_error(f"Agent cancel request exception: {e!s}", "Razyyn Cancel")
...
_DOWNLOAD_ENDPOINT: str = "/api/method/razyyn.razyyn.page.agent_chat.agent_chat.download_file"
