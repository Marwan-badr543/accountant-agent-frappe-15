...
class FileUploadHandler {
	...
	_render_attach_button() {
		this.$attach_btn = $(`
			<button class="agent-attach-btn" type="button" title="${__(
				"Attach files or images (up to 5 files)"
			)}">
	...
	_validate_batch(incoming_files) {
		...
		let rules = (selector &&
			typeof selector.get_rules === "function" &&
			selector.get_rules()) || {
			max_files: 5,
			max_per_file_mb: 1,
		...
		let agent_name =
			selector && selector.AGENT_DEFINITIONS && selector.AGENT_DEFINITIONS[agent_type]
		...
		if (total_count > rules.max_files) {
			frappe.show_alert(
				{
					message: __(
						"Maximum {0} files allowed for {1}. You currently have {2} attached and tried to add {3}.",
						[
							rules.max_files,
							agent_name,
							this.pending_attachments.length,
							incoming_files.length,
						]
					),
					indicator: "orange",
				},
		...
		for (let file of incoming_files) {
			if (!this._is_allowed_type(file.name)) {
				...
				frappe.show_alert(
					{
						message: __(
							'File "{0}" has unpermitted type ({1}). Only standard accounting document types are allowed.',
							[file.name, ext]
						),
						indicator: "red",
					},
			...
			for (let file of incoming_files) {
				if (file.size > max_bytes) {
					frappe.show_alert(
						{
							message: __(
								'"{0}" is {1} MB. For {2}, each file must not exceed {3} MB.',
								[
									file.name,
									(file.size / (1024 * 1024)).toFixed(2),
									agent_name,
									rules.max_per_file_mb,
								]
							),
							indicator: "orange",
						},
		...
		if (current_non_excel + new_non_excel > max_non_excel_bytes) {
			...
			frappe.show_alert(
				{
					message: __(
						"Total non-Excel files ({0} MB) exceed the {1} MB aggregate limit for {2}.",
		...
		if (current_excel + new_excel > max_excel_bytes) {
			...
			frappe.show_alert(
				{
					message: __(
						"Total Excel files ({0} MB) exceed the {1} MB aggregate limit for {2}.",
	...
	async _process_and_upload(file) {
		...
		let preview_id = this._add_preview_item(
			file.name,
			is_img ? "image" : "file",
			"uploading",
			file
	...
	_upload_to_server(file) {
		return new Promise((resolve, reject) => {
			...
			$.ajax({
				url: "/api/method/razyyn.razyyn.page.agent_chat.agent_chat.upload_agent_file",
				type: "POST",
				data: form_data,
				processData: false,
				contentType: false,
	...
	_update_preview_item(preview_id, status) {
		...
		if (status === "error") {
			$item.append(
				'<span class="agent-preview-status error"><i class="fa fa-exclamation-circle"></i></span>'
			);
		} else if (status === "success") {
			$item.append(
				'<span class="agent-preview-status success"><i class="fa fa-check-circle"></i></span>'
			);
			setTimeout(() => {
