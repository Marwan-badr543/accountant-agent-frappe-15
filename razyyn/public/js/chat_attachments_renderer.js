...
class ChatAttachmentsRenderer {
	constructor() {
		...
		this.FILE_PATTERN =
			/\[(?:FILE:)?([^:]+):(\/(?:private|files|api)[^\]]+|https?:\/\/[^\]]+)\]/g;
		this.IMAGE_PATTERN =
			/\[(?:IMAGE:)?([^:]+):(\/(?:private|files|api)[^\]]+|https?:\/\/[^\]]+)\]/g;
	}
	...
	parse_and_render(content) {
		...
		let clean_text = content.replace(this.FILE_PATTERN, "").replace(this.IMAGE_PATTERN, "");

		let escaped_file_pattern =
			/\[(?:FILE:)?([^:]+):(&#x2F;(?:private|files|api)[^\]]+|https?:\/\/[^\]]+)\]/g;
		let escaped_image_pattern =
			/\[(?:IMAGE:)?([^:]+):(&#x2F;(?:private|files|api)[^\]]+|https?:\/\/[^\]]+)\]/g;
	...
	has_attachments(content) {
		...
		this.IMAGE_PATTERN.lastIndex = 0;
		return (
			this.FILE_PATTERN.test(unescaped_content) || this.IMAGE_PATTERN.test(unescaped_content)
		);
	}
