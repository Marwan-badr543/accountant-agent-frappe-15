...
class AgentSelector {
	constructor(options = {}) {
		...
		this.AGENT_DEFINITIONS = {
			...
			analyse: {
				...
				description: __(
					"In-depth analysis. Max 5 files (10 MB total, Excel up to 20 MB)."
				),
				rules: {
					max_files: 5,
					max_per_file_mb: 20,
			...
			audit: {
				...
				description: __(
					"Compliance & Audit. Max 5 files (15 MB total, Excel up to 20 MB)."
				),
				rules: {
					max_files: 5,
					max_per_file_mb: 20,
			...
			reconcile: {
				...
				description: __(
					"Match two or more sources and explain every difference. Max 8 files (20 MB total, Excel up to 40 MB)."
				),
			...
			create: {
				...
				description: __(
					"Prepare and record entries in your ERP. Every document is saved as a draft for your approval. Max 3 files (10 MB total, Excel up to 20 MB)."
				),
	...
	render($parent) {
		...
		this.$container = $(`
			<div class="agent-dropdown-container">
				<button type="button" class="agent-dropdown-toggle-btn ${selected.badge_class}" title="${
			selected.description
				...
				<div class="agent-dropdown-menu" style="display: none;">
					${Object.keys(this.AGENT_DEFINITIONS)
						.map((key) => {
							let agent = this.AGENT_DEFINITIONS[key];
							let is_active = key === this.selected_agent;
							return `
							<div class="agent-dropdown-item ${is_active ? "active" : ""} ${agent.badge_class}" data-agent="${
								agent.id
							}">
	...
	set_agent(agent_id) {
		...
		if (this.$container) {
			...
			let $selected_item = this.$container.find(
				`.agent-dropdown-item[data-agent="${agent_id}"]`
			);
			...
			$selected_item
				.find(".agent-dropdown-item-header")
				.append('<i class="fa fa-check check-icon"></i>');
		}
