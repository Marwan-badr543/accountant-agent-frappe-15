...
class ChatMessageHandler {
	...
	async send_chat_message(message, file_urls = null) {
		...
		if (this.chat.session_manager.is_new_chat_draft) {
			...
			try {
				await frappe.xcall(
					"razyyn.razyyn.page.agent_chat.agent_chat.create_chat_with_id",
		...
		let agent_type = this.chat.agent_selector
			? this.chat.agent_selector.get_selected_agent()
			: "auto";

		try {
			let agent_email = localStorage.getItem("connected_agent_email");
			let res = await frappe.xcall("razyyn.razyyn.page.agent_chat.agent_chat.send_message", {
	...
	cancel_agent_execution() {
		...
		frappe
			.xcall("razyyn.razyyn.page.agent_chat.agent_chat.cancel_agent", {
