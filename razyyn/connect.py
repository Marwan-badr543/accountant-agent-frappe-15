...
"""Self-service onboarding: connect this ERP to the Razyyn platform.
...
from razyyn.install import AGENT_ROLE, AGENT_USER, ensure_agent_user
...
def _settings_doc(agent_email: str) -> Any:
	...
	"""
	from razyyn.razyyn.page.agent_chat.agent_chat import (
	...
	if not doc:
		frappe.throw(
			_("Please sign in to the Razyyn from the chat page first."),
...
def _public_site_url() -> str:
	...
	"""
	override = frappe.conf.get("razyyn_public_url")
...
def _platform_request(doc: Any, method: str, path: str, data: dict | None = None) -> dict:
	...
	try:
		...
		if response.status_code == 401:
			...
			if not refreshed:
				...
				frappe.throw(
					_("Your Razyyn session has expired. Please sign in " "again from the chat page."),
					frappe.AuthenticationError,
				)
		...
		frappe.log_error(
			title="Razyyn: platform unreachable",
			message=f"{method} {path} failed: {exc}",
		)
		frappe.throw(
			_("Could not reach the Razyyn service. Please try again " "in a moment."),
...
def get_write_connection_status(agent_email: str | None = None) -> dict:
	...
	if frappe.db.exists("DocType", "Agent Write Policy"):
		...
		status["allowed_doctypes"] = [row.document_type for row in (policy.allowed_document_types or [])]
...
def _missing_steps(status: dict) -> list[str]:
	...
	if not status["connected_to_platform"]:
		missing.append("Press Connect to link this ERP to your Razyyn account.")
	...
	if not writable:
		missing.append(
			...
			"document. Grant the 'Razyyn' role Create and Write on the "
...
@frappe.whitelist()
def connect_write_access(agent_email: str, enable_recording: int | str = 0) -> dict:
	"""Link this ERP to the caller's Razyyn account. Idempotent.
	...
	return {
		...
		"recording_enabled": bool(result.get("write_enabled")),
		"message": (
			_("Connected. Your ERP and the Razyyn can now reach each other.")
			if verified
...
def set_recording_enabled(agent_email: str, enabled: int | str) -> dict:
	...
	if not connection_id:
		frappe.throw(
			_("Connect this ERP to your Razyyn account first."),
			frappe.ValidationError,
		)
	...
	result = _platform_request(
		doc,
		"POST",
		f"/api/create/connections/{connection_id}/write-enabled",
...
def rotate_and_reconnect(agent_email: str) -> dict:
	...
	return {
		...
		"verification_error": result.get("verification_error"),
		"message": _(
			"New credentials issued. Any other Razyyn account connected "
...
def disconnect_write_access(agent_email: str) -> dict:
	...
	if connection_id:
		_platform_request(
			doc,
			"DELETE",
			f"/api/create/connections/{connection_id}",
		)
	...
	frappe.db.set_value(
		...
		{
			"erp_connection_id": "",
			"write_recording_enabled": 0,
			"write_connected_on": None,
			"write_last_error": "",
		},
...
def apply_recommended_setup() -> dict:
	...
	if not any(row.document_type == _RECOMMENDED_WRITABLE for row in (policy.allowed_document_types or [])):
		policy.append(
...
def _grant(add_permission, update_permission_property, doctype: str, perms: dict) -> None:
	...
	if not frappe.db.exists("Custom DocPerm", {"parent": doctype, "role": AGENT_ROLE, "permlevel": 0}):
