...
from razyyn.agent_api.db.agent_api_repository import backfill_api_key_hashes

# ── Constants ────────────────────────────────────────────────────────────────

AGENT_ROLE: str = "Razyyn"
...
def provision_agent_access() -> None:
	...
	frappe.db.commit()
	frappe.logger("razyyn").info("Razyyn access provisioned (user=%s, role=%s).", AGENT_USER, AGENT_ROLE)
...
def ensure_app_owned_permissions() -> None:
	...
	for doctype, permissions in APP_OWNED_PERMISSIONS:
		...
		existing = frappe.db.exists("Custom DocPerm", {"parent": doctype, "role": AGENT_ROLE, "permlevel": 0})
		if not existing:
...
def ensure_workspace_shortcut() -> None:
	...
	try:
		...
		frappe.logger("razyyn").warning("Could not add the Agent Write Log workspace shortcut: %s", exc)
...
def get_agent_status() -> dict[str, object]:
	...
	user = frappe.get_doc("User", AGENT_USER)
	policy_enabled = (
		bool(frappe.db.get_single_value("Agent Write Policy", "enabled"))
			...
			results.append(
				{
					...
					"create": bool(row.allow_create and frappe.has_permission(row.document_type, "create")),
					"submit": bool(row.allow_submit and frappe.has_permission(row.document_type, "submit")),
					"cancel": bool(row.allow_cancel and frappe.has_permission(row.document_type, "cancel")),
					"amend": bool(row.allow_amend and frappe.has_permission(row.document_type, "amend")),
				}
			)
	finally:
