...
class TestSessionOwnershipGuard(TestCase):
	def test_owner_match_passes(self):
		with patch(
			"accountant_agent.agent_api.services.agent_api_service.get_chat_session_owner",
	...
	def test_process_clarification_request_rejects_foreign_session(self):
		with (
			...
			patch(
				"accountant_agent.agent_api.services.agent_api_service.insert_chat_history_record"
			) as insert_mock,
		):
			with self.assertRaises(ResourceNotFoundError):
