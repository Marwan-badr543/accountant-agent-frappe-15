...
-----------------------------------
Sends email and Telegram messages on the company's behalf, from credentials
that live in this site and never leave it.
...
#: stays under both with room for base64 overhead, which inflates by a third.
...
class UnknownDestinationError(MessagingError):
	"""The named Telegram destination is not one the customer listed.

	A bot cannot start a conversation, so the agent cannot be allowed to invent
	a chat id: an unlisted destination is not a delivery failure, it is a
...
class ProviderRefusedError(MessagingError):
	"""Google or Telegram rejected the message."""
	...
	for url in file_urls or []:
		if not url or not str(url).startswith("/"):
			raise AttachmentError(
				f"'{url}' is not a file on this site. Only files this site "
				"already holds can be attached."
			)
...
def _gmail_credentials(settings):
	...
	try:
		...
		raise ChannelNotConfiguredError(
			"gmail", f"The Google service account key was not accepted: {exc}",
		) from exc
...
def _send_gmail(settings, to: str, subject: str, body: str, attachments: list[dict]) -> str:
	...
	if response.status_code >= 400:
		raise ProviderRefusedError(
			f"Gmail refused the message ({response.status_code}): "
	...
	if not message_id:
		...
		raise ProviderRefusedError(
			"Gmail accepted the request but returned no message id, so the send "
			"cannot be confirmed.",
...
def _resolve_telegram_destination(settings, destination: str | None) -> tuple[str, str]:
	...
	if not destination:
		default = next((r for r in rows if r.is_default), None) or (
	...
	try:
		...
		if channel == "gmail":
			...
			if not destination or "@" not in str(destination):
				raise UnknownDestinationError(
					f"'{destination}' is not an email address."
				)
			resolved_destination = str(destination).strip()
			provider_message_id = _send_gmail(
				settings, resolved_destination, subject or "", body or "", attachments,
			)
		else:
			if not settings.telegram_enabled:
				raise ChannelNotConfiguredError("telegram", _telegram_gap(settings, []))
			...
			provider_message_id = _send_telegram(
				settings, resolved_destination, _telegram_body(subject, body), attachments,
			)

	except MessagingError as exc:
		_record(
			idempotency_key, channel, "REFUSED" if _is_refusal(exc) else "FAILED",
			resolved_destination, label, subject, body, file_urls,
			requested_by, approved_by, run_id, session_id,
			provider_message_id="", error_code=exc.code, error_message=exc.detail,
		)
		_remember_channel_error(settings, channel, exc.detail)
		raise
	except Exception as exc:
		_record(
			idempotency_key, channel, "FAILED",
			resolved_destination, label, subject, body, file_urls,
			requested_by, approved_by, run_id, session_id,
			provider_message_id="", error_code="UNEXPECTED", error_message=str(exc)[:500],
		)
		raise

	_record(
		idempotency_key, channel, "SENT",
		resolved_destination, label, subject, body, file_urls,
		requested_by, approved_by, run_id, session_id,
		provider_message_id=provider_message_id, error_code="", error_message="",
	)
...
def _telegram_body(subject: str | None, body: str | None) -> str:
	"""Telegram has no subject line, so a subject becomes the first line.
...
def _record(
	idempotency_key, channel, status, destination, destination_label,
	subject, body, file_urls, requested_by, approved_by, run_id, session_id,
	...
	try:
		frappe.get_doc({
			...
			"attachment_names": ", ".join(
				os.path.basename(u) for u in (file_urls or [])
			)[:500],
...
def _remember_channel_error(settings, channel: str, detail: str) -> None:
	...
	"""
	field = "gmail_last_error" if channel == "gmail" else "telegram_last_error"
	try:
